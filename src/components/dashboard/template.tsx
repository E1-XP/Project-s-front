...
export const DashboardComponent: ComponentType<Props> = ({ handleMessageSubmit,
    handleRoomClick, goToCreateRoom, users, rooms, messages }) => {
                            ...
                            {Object.keys(rooms.list).length ?
                                Object.keys(rooms.list).map((itm) =>
                                    <ListItem key={itm} data-id={itm} onClick={handleRoomClick}>
                                        ...
                                        {rooms.list[itm].isPrivate && <ListItemText secondary={'private'} />}
                                    </ListItem>) :
                                <ListItem key={0}>  no rooms created </ListItem>}
                        </List>
                        <Button variant="contained" color="primary" onClick={goToCreateRoom} >
                            Create new room
